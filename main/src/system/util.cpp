...
std::string get_matter_attribute_value_string(esp_matter_attr_val_t value)
{
    char temp[64] = {0,};
    switch (value.type) {
    ...
    case ESP_MATTER_VAL_TYPE_NULLABLE_INTEGER:
        snprintf(temp, sizeof(temp), "%d", value.val.i);
        break;
    case ESP_MATTER_VAL_TYPE_FLOAT:
    case ESP_MATTER_VAL_TYPE_NULLABLE_FLOAT:
        snprintf(temp, sizeof(temp), "%g", value.val.f);
        break;
    case ESP_MATTER_VAL_TYPE_ARRAY:
        snprintf(temp, sizeof(temp), "array");
        break;
